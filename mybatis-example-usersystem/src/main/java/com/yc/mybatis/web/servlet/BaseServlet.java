...
public class BaseServlet extends HttpServlet {
	...
	protected SmartUpload getSmartUpload(HttpServletRequest req, HttpServletResponse resp) {
		try {
			...
			su.upload();
			return su;
		} catch (SmartUploadException | ServletException | IOException e) {
	...
	//class 为类，接口的类型，类似int 为整型的类型
		//UserServlet.class的类型是class，类似1 的类型是int
		//Class为类的抽象
		protected <T> T getReqParam2Obj(HttpServletRequest request, Class<T> clazz) {
			...
			while(paramNames.hasMoreElements()){
				...
				try {
					Class<?> paramType = clazz.getMethod("g"+suff).getReturnType();//取到getXxx()方法的返回类型的类对象
					...
					LogManager.getLogger().warn("在"+clazz.getName()+"中没有对应"+e.getMessage());
